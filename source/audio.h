...
public:
  ...
  void init_alsa(const char *playdev, const char *captdev, unsigned int fsamp,
                 int frsize, int nfrags, int ncapt, int nplay);
  void init_jack(const char *server);

  // WAV Recording control
  size_t predict_memory_requirements(float original_sample_rate,
                                     int decimation_factor);
  bool start_wav_recording(char _rec_filename[128], float original_sample_rate,
...
private:
  ...
  void init_lpf_filter(unsigned int sample_rate, float host_freq,
                       float cutoff_freq);
  void demodulate_buffer(float *input_buffer, float *output_buffer_demulated,
                         int num_samples);
