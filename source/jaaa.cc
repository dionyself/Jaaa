...
// Define CLI options for resource desc of X11 (Xrm)
...
int main(int argc, char *argv[]) {
  ...
  // Initialse resource database and process args
  xrm.init(&argc, argv, CP "jaaa", options, NOPTS);
  if (xrm.getb(".help", 0))
    help();

  // Open display (X11 graphics connection)
  ...
  // Open audio interface (JACK o ALSA depending on arg)
  ...
  if (xrm.getb(".jack", 0)) {
    ...
    p = xrm.get(".fsamp", 0);
    if (!p || sscanf(p, "%d", &fsamp) != 1)
      fsamp = 48000;
    p = xrm.get(".period", 0);
    if (!p || sscanf(p, "%d", &period) != 1)
      period = 1024;
    p = xrm.get(".nfrags", 0);
    if (!p || sscanf(p, "%d", &nfrags) != 1)
      nfrags = 2;
  ...
  // Initialise resources, visual styles and create windows
  ...
  // Try to lock memory to avoid paging fails
  if (mlockall(MCL_CURRENT | MCL_FUTURE)) {
    fprintf(stderr, "Warning: memory lock failed.\n");
  }

  // Enter main loop (Inter-Thread Communication / GUI)
  while (mainwin->running()) {
    switch (itcc.get_event()) {
    case EV_TRIG: // New audio pgk processed event
      mainwin->handle_trig();
      rootwin->handle_event();
      XFlush(display->dpy());
      break;

    case EV_JACK: // JACK server closed event
      mainwin->handle_term();
      break;

    case EV_MESG: // Audio backend msg event
      mainwin->handle_mesg(itcc.get_message());
      rootwin->handle_event();
      XFlush(display->dpy());
      break;

    case EV_X11: // GUI events (clics, redraw)
      rootwin->handle_event();
      xhandler->next_event();
      break;
    }
